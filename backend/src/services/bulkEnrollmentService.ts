...
export const parseBulkExcel = async (filePath: string): Promise<ParsedBulkRow[]> => {
  ...
  sheetRows.forEach((excelRow, index) => {
    ...
    const payload: RegisterAndEnrollPayload = {
      ...
      phone1: (mother?.phone || father?.phone || representativeData?.phone || '') || null,
      phone2: (mother?.phone2 || father?.phone2 || representativeData?.phone2 || '') || null,
      email: sanitizeString(normalized.email) || null,
      address: sanitizeString(normalized.address) || null,
      whatsapp: (mother?.whatsapp || mother?.phone || father?.whatsapp || father?.phone || representativeData?.whatsapp || representativeData?.phone || '') || null,
      previousSchoolIds,
      mother,
      father,
