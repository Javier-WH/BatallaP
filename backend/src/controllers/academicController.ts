...
export const createSubject = async (req: Request, res: Response) => {
  const { name, subjectGroupId, usesLiteralGrades, abbreviation, icon, color, allowConsecutiveBlocks, maxHoursPerDay, difficulty, weeklyBlocks } = req.body as { name: string; subjectGroupId?: number | null; usesLiteralGrades?: boolean; abbreviation?: string | null; icon?: string | null; color?: string | null; allowConsecutiveBlocks?: number; maxHoursPerDay?: number | null; difficulty?: string | null; weeklyBlocks?: number | null };
  const subject = await Subject.create({ name, subjectGroupId: subjectGroupId ?? null, usesLiteralGrades: usesLiteralGrades ?? false, abbreviation: abbreviation ?? null, icon: icon ?? null, color: color ?? null, allowConsecutiveBlocks: allowConsecutiveBlocks ?? 0, maxHoursPerDay: maxHoursPerDay ?? null, difficulty: difficulty ?? 'medium', weeklyBlocks: weeklyBlocks ?? null });
  res.json(subject);
};
...
export const updateSubject = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { name, subjectGroupId, usesLiteralGrades, abbreviation, icon, color, allowConsecutiveBlocks, maxHoursPerDay, difficulty, weeklyBlocks } = req.body as { name?: string; subjectGroupId?: number | null; usesLiteralGrades?: boolean; abbreviation?: string | null; icon?: string | null; color?: string | null; allowConsecutiveBlocks?: number; maxHoursPerDay?: number | null; difficulty?: string | null; weeklyBlocks?: number | null };
    await Subject.update({ name, subjectGroupId: subjectGroupId ?? null, usesLiteralGrades: usesLiteralGrades ?? false, abbreviation: abbreviation ?? null, icon: icon ?? null, color: color ?? null, allowConsecutiveBlocks: allowConsecutiveBlocks ?? 0, maxHoursPerDay: maxHoursPerDay ?? null, difficulty: difficulty ?? 'medium', weeklyBlocks: weeklyBlocks ?? null }, { where: { id } });
...
export const toggleSubjectNotRepairable = async (req: Request, res: Response) => {
  try {
    const { periodGradeId, subjectId, notRepairable } = req.body as {
      periodGradeId: number;
      subjectId: number;
    ...
    await pgs.update({ notRepairable });
