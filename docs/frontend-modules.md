...
| `shared/ConstanciaEditor.tsx` | Editor Tiptap de plantillas con inserción de variables `{{...}}`; las claves parametrizadas (`subject.{n}.*`, `term.{m}`) piden el número al insertar. Incluye objetos flotantes (`FloatingLine`, `FloatingTable`, helpers en `floatingObject.ts`): líneas horizontales/verticales y tablas movibles/redimensionables, delante o detrás del texto («Objetos detrás» permite seleccionar las de atrás). Cada objeto tiene modo de anclaje: «Anclada al texto» (por defecto al insertar) guarda `top` como desplazamiento respecto al párrafo bajo el objeto — al moverlo, el ancla se reubica junto al párrafo que toca, así sigue al texto cuando las variables cambian su largo; «Fija en la página» usa coordenadas absolutas. Filas/columnas de tabla: botones «+Fila/-Fila/+Col/-Col» (usan la celda activa) o clic derecho en una celda; el selector de inserción acepta hasta 60×12. Aparecen en vista previa/PDF; la exportación a Word las omite. |
