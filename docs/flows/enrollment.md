...
- **Documentos solo numéricos**: las cédulas `Venezolano`/`Extranjero` y la `Cedula Escolar` se guardan **solo con dígitos** (estudiantes y representantes). La letra V-/E- se deriva de `documentType` al mostrarla. Lo aplica `normalizeDocumentNumber` (`backend/src/utils/documentNumber.ts`) en los hooks de `Person` y `GuardianProfile` y en las búsquedas por documento. Los pasaportes conservan sus caracteres. La Cédula Escolar autogenerada es `orden de nacimiento + año (2 dígitos) + cédula de la madre`, sin letra. La migración `20261123120000-documents-digits-only` normaliza los datos existentes y omite (con log) colisiones o valores sin dígitos.
