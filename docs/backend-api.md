...
- **Compactación del día del salón**: cada día usado por una sección se penaliza si forma más de un tramo (`day_compactness_weight`, default `200`) o si tiene menos de `min_consolidated_blocks_per_day` bloques (`thin_day_weight`, default `150`; el mínimo se edita en el panel «Excepciones», default `2`). En la práctica: horarios continuos y sin venir solo a la mañana y volver a la tarde.
- **Dificultad de materias** (`Subject.difficulty`, override en `ScheduleException.difficulty`): `heavy` evita bloques adyacentes con otra pesada en el mismo turno (`heavy_back_to_back_weight`, default `80`) y los últimos N bloques de cada turno (`heavy_late_block_weight`, default `40`; N = `heavy_avoid_last_n_morning`/`heavy_avoid_last_n_afternoon`, default `1`). `light`/`medium` no tienen penalización propia.
...
- Los pesos se editan en Control de Estudios → Configuración Académica → «Ajustes avanzados del generador de horarios». Orden de prioridad del objetivo: colocar todo → bloques consecutivos → bloques forzados → compactación → dificultad → preferencias de profesor → distribuir entre días → sin huecos por turno → bloques tempranos.
