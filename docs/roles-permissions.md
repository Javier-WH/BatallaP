...
- **Escrituras de inscripción/matrícula** (`inscriptionController`): gates de sesión por helper —
  - `canEnrollStudent` (`Master`/`Administrador`): `POST /api/inscriptions`, `POST /api/inscriptions/register`, `POST /api/inscriptions/quick-register`, `DELETE /api/inscriptions/:id`, `POST /api/inscriptions/:id/withdraw`, `POST /api/inscriptions/:id/reactivate`. También `bulkEnrollmentController`: `POST /api/inscriptions/bulk/preview`, `POST /api/inscriptions/bulk/process`, `POST /api/inscriptions/bulk/retry-single` (el `GET /template` queda abierto).
