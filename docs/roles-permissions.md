...
- **Escrituras de inscripción/matrícula** (`inscriptionController`): gates de sesión por helper —
  - `canEnrollStudent` (`Master`/`Administrador`): `POST /api/inscriptions`, `POST /api/inscriptions/register`, `POST /api/inscriptions/quick-register`, `DELETE /api/inscriptions/:id`, `POST /api/inscriptions/:id/withdraw`, `POST /api/inscriptions/:id/reactivate`, `POST /api/matriculations/:id/withdraw`, `POST /api/matriculations/:id/reactivate` (Retirar/Reactivar — Reactivar deja al estudiante en "No Matriculados" sin sección). También `bulkEnrollmentController`: `POST /api/inscriptions/bulk/preview`, `POST /api/inscriptions/bulk/process`, `POST /api/inscriptions/bulk/retry-single` (el `GET /template` queda abierto).
  - `canManageEnrollment` (`Master`/`Administrador`/`Control de Estudios`): `POST /api/matriculations/:id/enroll`, `POST /api/inscriptions/:id/unmatriculate`, `PUT`/`PATCH /api/inscriptions/:id`, `PATCH /api/matriculations/:id`, `POST`/`DELETE /api/inscriptions/:id/subjects`, `PUT`/`POST /api/inscriptions/:id/group-choices*`.
  - Visibilidad de matrícula (`PATCH /api/matriculations/:id/visibility`, `POST /api/matriculations/bulk-visibility`): `isPrivilegedUser` — `Master`/`Administrador` solamente.
