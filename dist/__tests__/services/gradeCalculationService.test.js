...
describe('GradeCalculationService — pure functions', () => {
    describe('calculateAccumulatedTermScore', () => {
        it('retorna roundFinalGrade del score del term', () => {
            ...
            expect(gradeCalculationService_1.GradeCalculationService.calculateAccumulatedTermScore(1, termGrades)).toBe(15);
            expect(gradeCalculationService_1.GradeCalculationService.calculateAccumulatedTermScore(2, termGrades)).toBe(9);
        });
        it('retorna MIN_FINAL_GRADE (1) cuando el term no existe', () => {
            const termGrades = [{ termId: 1, score: 10 }];
            expect(gradeCalculationService_1.GradeCalculationService.calculateAccumulatedTermScore(99, termGrades)).toBe(gradeEvaluationService_1.MIN_FINAL_GRADE);
        });
        it('retorna MIN_FINAL_GRADE (1) cuando el score es 0', () => {
            const termGrades = [{ termId: 1, score: 0 }];
            expect(gradeCalculationService_1.GradeCalculationService.calculateAccumulatedTermScore(1, termGrades)).toBe(gradeEvaluationService_1.MIN_FINAL_GRADE);
        });
    });
    describe('calculateFinalTermScore', () => {
        it('retorna null cuando councilDone=false', () => {
            const termGrades = [{ termId: 1, score: 15 }];
            expect(gradeCalculationService_1.GradeCalculationService.calculateFinalTermScore(1, termGrades, false)).toBeNull();
        });
        it('retorna el score cuando councilDone=true', () => {
            const termGrades = [{ termId: 1, score: 14.5 }];
            expect(gradeCalculationService_1.GradeCalculationService.calculateFinalTermScore(1, termGrades, true)).toBe(15);
        });
    });
    describe('calculateFinalScore', () => {
        it('usa SubjectFinalGrade directo para transferencia', () => {
            const result = gradeCalculationService_1.GradeCalculationService.calculateFinalScore([{ termId: 1, finalScore: 5 }], { finalScore: 18, gradeType: 'transferencia' });
            expect(result).toBe(18);
        });
        it('usa SubjectFinalGrade directo para equivalencia', () => {
            const result = gradeCalculationService_1.GradeCalculationService.calculateFinalScore([{ termId: 1, finalScore: 5 }], { finalScore: 12, gradeType: 'equivalencia' });
            expect(result).toBe(12);
        });
        it('aplica MIN_FINAL_GRADE a transferencia con score 0', () => {
            const result = gradeCalculationService_1.GradeCalculationService.calculateFinalScore([], { finalScore: 0, gradeType: 'transferencia' });
            expect(result).toBe(gradeEvaluationService_1.MIN_FINAL_GRADE);
        });
        it('retorna null para transferencia sin finalScore', () => {
            const result = gradeCalculationService_1.GradeCalculationService.calculateFinalScore([], { finalScore: null, gradeType: 'transferencia' });
            expect(result).toBeNull();
        });
        it('usa SubjectFinalGrade directo cuando isClosedPeriod=true', () => {
            const result = gradeCalculationService_1.GradeCalculationService.calculateFinalScore([{ termId: 1, finalScore: 5 }], { finalScore: 14, gradeType: 'regular' }, { isClosedPeriod: true });
            expect(result).toBe(14);
        });
        it('usa SubjectFinalGrade cuando todos los lapsos están done', () => {
            const result = gradeCalculationService_1.GradeCalculationService.calculateFinalScore([
        ...
        it('retorna null cuando no hay lapsos', () => {
            const result = gradeCalculationService_1.GradeCalculationService.calculateFinalScore([], null);
            expect(result).toBeNull();
        });
    });
    describe('calculateGeneralAverage', () => {
        it('mode=final: promedia subjects con includeInAverage=true', () => {
            ...
            expect(gradeCalculationService_1.GradeCalculationService.calculateGeneralAverage(subjects, 'final')).toBe(12);
        });
        it('mode=final: aplica MIN_FINAL_GRADE por subject', () => {
            ...
            // (max(1,0) + 14) / 2 = 7.5
            expect(gradeCalculationService_1.GradeCalculationService.calculateGeneralAverage(subjects, 'final')).toBe(7.5);
        });
        it('mode=final: ignora subjects con finalScore=null', () => {
            ...
            expect(gradeCalculationService_1.GradeCalculationService.calculateGeneralAverage(subjects, 'final')).toBe(14);
        });
        it('mode=final: retorna null si no hay scores', () => {
            const subjects = [
                { finalScore: null, includeInAverage: true },
            ];
            expect(gradeCalculationService_1.GradeCalculationService.calculateGeneralAverage(subjects, 'final')).toBeNull();
        });
        it('mode=accumulated: promedia los accumulatedScores', () => {
            const scores = [10, 14, 0];
            // max(1,10)=10, max(1,14)=14, max(1,0)=1 → (10+14+1)/3 = 8.33
            expect(gradeCalculationService_1.GradeCalculationService.calculateGeneralAverage([], 'accumulated', scores)).toBe(8.33);
        });
        it('mode=accumulated: retorna null si no hay scores', () => {
            expect(gradeCalculationService_1.GradeCalculationService.calculateGeneralAverage([], 'accumulated', [])).toBeNull();
            expect(gradeCalculationService_1.GradeCalculationService.calculateGeneralAverage([], 'accumulated', undefined)).toBeNull();
        });
    });
    describe('resolveStatus', () => {
        it('retorna "reprobada" para null', () => {
            expect(gradeCalculationService_1.GradeCalculationService.resolveStatus(null, 10)).toBe('reprobada');
        });
        it('retorna "reprobada" para undefined', () => {
            expect(gradeCalculationService_1.GradeCalculationService.resolveStatus(undefined, 10)).toBe('reprobada');
        });
        it('retorna "aprobada" cuando pasa', () => {
            expect(gradeCalculationService_1.GradeCalculationService.resolveStatus(10, 10)).toBe('aprobada');
            expect(gradeCalculationService_1.GradeCalculationService.resolveStatus(15, 10)).toBe('aprobada');
        });
        it('retorna "reprobada" cuando no pasa', () => {
            expect(gradeCalculationService_1.GradeCalculationService.resolveStatus(9, 10)).toBe('reprobada');
        });
    });
    describe('buildCouncilDoneChecker', () => {
        it('construye un mapa correcto de sections→terms done', () => {
            ...
            const checker = gradeCalculationService_1.GradeCalculationService.buildCouncilDoneChecker(checklists);
        ...
        it('retorna false para listas vacías', () => {
            const checker = gradeCalculationService_1.GradeCalculationService.buildCouncilDoneChecker([]);
    ...
    describe('buildTermGradesWithFallback', () => {
        it('usa el stored score cuando > 0', () => {
            const termGrades = [{ termId: 1, score: 15 }];
            const result = gradeCalculationService_1.GradeCalculationService.buildTermGradesWithFallback(termGrades, [], [], [1]);
        ...
        it('usa fallback cuando stored es 0', () => {
            ...
            const result = gradeCalculationService_1.GradeCalculationService.buildTermGradesWithFallback(termGrades, qualifications, [], [1]);
        ...
        it('usa fallback cuando no hay stored', () => {
            ...
            const result = gradeCalculationService_1.GradeCalculationService.buildTermGradesWithFallback([], qualifications, [], [1]);
        ...
        it('suma councilPoints al fallback', () => {
            ...
            const result = gradeCalculationService_1.GradeCalculationService.buildTermGradesWithFallback([], [], councilPoints, [1]);
