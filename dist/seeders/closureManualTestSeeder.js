...
function seedClosureManualTest() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            // 1. Settings
            yield index_1.Setting.findOrCreate({
            ...
            let nextPeriod = yield index_1.SchoolPeriod.findOne({
                where: { status: 'preinscripcion' },
                transaction: t,
            });
            if (!nextPeriod) {
                nextPeriod = yield index_1.SchoolPeriod.create({
                    period: '2026-2027',
            ...
            yield index_1.Term.update({ isBlocked: true, isActive: false }, { where: { schoolPeriodId: activePeriod.id }, transaction: t });
            console.log(`🔒 ${terms.length} lapsos bloqueados`);
            // 6. Get role Alumno
            const studentRole = yield index_1.Role.findOne({ where: { name: 'Alumno' }, transaction: t });
            if (!studentRole) {
            ...
            for (let i = 0; i < grades.length; i++) {
                ...
                const existing = yield index_1.SchoolPeriodTransitionRule.findOne({
                    where: { gradeFromId: gradeFrom.id },
                    transaction: t,
                });
                if (!existing) {
                    yield index_1.SchoolPeriodTransitionRule.create({
                        gradeFromId: gradeFrom.id,
                        gradeToId: (_a = gradeTo === null || gradeTo === void 0 ? void 0 : gradeTo.id) !== null && _a !== void 0 ? _a : null,
                        minAverage: 10,
            ...
            for (const grade of grades) {
                ...
                const pgSections = yield index_1.PeriodGradeSection.findAll({
                    where: { periodGradeId: pg.id },
                    transaction: t,
                });
                for (const pgs of pgSections) {
                    for (const term of terms) {
                        yield index_1.CouncilChecklist.findOrCreate({
                            where: {
            ...
            const existingRevision = yield index_1.RevisionPeriod.findOne({
                where: { schoolPeriodId: activePeriod.id },
                transaction: t,
            });
            if (existingRevision && existingRevision.status === 'open') {
                yield existingRevision.update({ status: 'pending' }, { transaction: t });
            }
            console.log('✅ Período de revisiones en estado pending');
            // 10. Ensure next period has structure (PeriodGrade + sections + subjects)
            for (const grade of grades) {
                let pgNext = yield index_1.PeriodGrade.findOne({
                    where: { schoolPeriodId: nextPeriod.id, gradeId: grade.id },
                    transaction: t,
                });
                if (!pgNext) {
                    pgNext = yield index_1.PeriodGrade.create({
                        schoolPeriodId: nextPeriod.id,
                        gradeId: grade.id,
                    }, { transaction: t });
                }
                // Copy sections from active period
                const pgCurrent = yield index_1.PeriodGrade.findOne({
                    where: { schoolPeriodId: activePeriod.id, gradeId: grade.id },
                    transaction: t,
                });
                if (!pgCurrent)
                    continue;
                const currentSections = yield index_1.PeriodGradeSection.findAll({
                ...
                const currentSubjects = yield index_1.PeriodGradeSubject.findAll({
                    where: { periodGradeId: pgCurrent.id },
                    transaction: t,
                });
            ...
            for (const studentCase of STUDENT_CASES) {
                ...
                const pgSection = yield index_1.PeriodGradeSection.findOne({
                    where: { periodGradeId: pg.id },
                    transaction: t,
                });
                if (!pgSection) {
                ...
                const section = yield index_1.Section.findByPk(pgSection.sectionId, { transaction: t });
                if (!section)
                    continue;
                // Check if student already exists (idempotency)
                const existingPerson = yield index_1.Person.findOne({
                ...
                for (let si = 0; si < orderedSubjects.length; si++) {
                    ...
                    const score = (_b = studentCase.scores[si]) !== null && _b !== void 0 ? _b : 15; // default 15 if not specified
...
exports.default = seedClosureManualTest;
